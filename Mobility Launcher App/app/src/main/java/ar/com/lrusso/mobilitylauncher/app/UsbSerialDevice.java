...
public abstract class UsbSerialDevice implements UsbSerialInterface
{
    ...
    protected UsbSerialBuffer serialBuffer;
    ...
    protected UsbSerialInputStream inputStream;
    protected UsbSerialOutputStream outputStream;
    ...
    public UsbSerialDevice(UsbDevice device, UsbDeviceConnection connection)
    {
        ...
        this.asyncMode = true;
        serialBuffer = new UsbSerialBuffer(mr1Version);
    }
    ...
    public static UsbSerialDevice createUsbSerialDevice(UsbDevice device, UsbDeviceConnection connection, int iface)
    {
        ...
        if(UsbSerialFTDISioIds.isDeviceSupported(vid, pid))
            return new UsbSerialFTDISerialDevice(device, connection, iface);
        else if(UsbSerialCP210xIds.isDeviceSupported(vid, pid))
            return new UsbSerialCP2102SerialDevice(device, connection, iface);
        else if(UsbSerialPL2303Ids.isDeviceSupported(vid, pid))
            return new UsbSerialPL2303SerialDevice(device, connection, iface);
        else if(UsbSerialCH34xIds.isDeviceSupported(vid, pid))
            return new UsbSerialCH34xSerialDevice(device, connection, iface);
        else if(isCdcDevice(device))
            return new UsbSerialCDCSerialDevice(device, connection, iface);
        else
            return null;
    }
    ...
    public int read(UsbReadCallback mCallback)
    {
        ...
        if(mr1Version)
        {
            if (workerThread != null) {
                ...
                workerThread.getUsbRequest().queue(serialBuffer.getReadBuffer(), UsbSerialBuffer.DEFAULT_READ_BUFFER_SIZE);
            }
        }else
        {
    ...
    public UsbSerialInputStream getInputStream() {
        if(asyncMode)
    ...
    public UsbSerialOutputStream getOutputStream() {
        if(asyncMode)
    ...
    {
        return (this instanceof UsbSerialFTDISerialDevice);
    }
        ...
        public void run()
        {
            while(working.get())
            {
                        ...
                        && request.getEndpoint().getDirection() == UsbConstants.USB_DIR_IN)
                {
                    ...
                    if(isFTDIDevice())
                    {
                        ((UsbSerialFTDISerialDevice) usbSerialDevice).ftdiUtilities.checkModemStatus(data); //Check the Modem status
                        serialBuffer.clearReadBuffer();

                        if(data.length > 2)
                        {
                            data = ((UsbSerialFTDISerialDevice) usbSerialDevice).ftdiUtilities.adaptArray(data);
                            onReceivedData(data);
                        }
                    }else
                    {
                    ...
                    // Queue a new request
                    requestIN.queue(serialBuffer.getReadBuffer(), UsbSerialBuffer.DEFAULT_READ_BUFFER_SIZE);
                }
            }
        }
            ...
            while(working.get())
            {
                ...
                if(inEndpoint != null)
                    numberBytes = connection.bulkTransfer(inEndpoint, serialBuffer.getBufferCompatible(),
                            UsbSerialBuffer.DEFAULT_READ_BUFFER_SIZE, 0);
                else
                    numberBytes = 0;
                ...
                if(numberBytes > 0)
                {
                    ...
                    if(isFTDIDevice())
                    {
                        ((UsbSerialFTDISerialDevice) usbSerialDevice).ftdiUtilities.checkModemStatus(dataReceived);

                        if(dataReceived.length > 2)
                        {
                            dataReceived = ((UsbSerialFTDISerialDevice) usbSerialDevice).ftdiUtilities.adaptArray(dataReceived);
